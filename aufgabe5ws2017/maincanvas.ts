...
namespace CANVAS_5 {
    ...
    function init(): void {
        ...
        for (let i: number = 0; i < 1; i++) {
            let lassoles: Sonne;
            lassoles = new Sonne(Math.random() * 660, Math.random() * 50, "orange", "#FFFF00");
            sol[i] = lassoles;
        ...
        for (let i: number = 0; i < 1; i++) {
            ...
            clouds = new SecondCloud(Math.random() * 200 + 20, Math.random() * 130, "grey", "grey");
            cloudsX[i] = clouds;
        ...
        for (let i: number = 0; i < 126; i++) {
            let s: Snowflakes = new Snowflakes(0 + Math.random() * 800, 0 + Math.random() * 600, "black", "whitesmoke");
            snow[i] = s;
    ...
    function animate(): void {
        ...
        for (let i: number = 0; i < sol.length; i++) {
            let sun: Sonne = sol[i];
            sun.drawSun();
            sun.animatedSun();
        ...
        for (let i: number = 0; i < cloudsX.length; i++) {
            let clouds: SecondCloud = cloudsX[i];
            clouds.drawCloud1();
