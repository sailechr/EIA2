...
namespace CANVAS4 {
    ...
    function drawrandomSnowflakes(): void {
        let numberofSnowflakes: number = 90;

        // draw randomly placed Snowflakes
        for (let n: number = 0; n < numberofSnowflakes; n++) {
            let x: number = 0.4 + (Math.random() * 770);
            let y: number = 1 + (Math.random() * 300);
